...
Description: BOSCH G-Sensor BMA253 driver implementation
...
static bool BMA253Initialized = false;

static uint8_t BMA253_RESOLUTION = BMA2x2_12_RESOLUTION;

static BMA253IrqHandler *BMA253Callback1 = NULL;
static BMA253IrqHandler *BMA253Callback2 = NULL;

void BMA253InterruptSignal1( void )
{
    if(BMA253Callback1 != NULL)
        BMA253Callback1();
}

void BMA253InterruptSignal2( void )
{
    if(BMA253Callback2 != NULL)
        BMA253Callback2();
}

void BMA253SetCallback(BMA253IrqHandler *irqHandler1, BMA253IrqHandler *irqHandler2 )
{
    BMA253Callback1 = irqHandler1;
    BMA253Callback2 = irqHandler2;
}

uint8_t BMA253Reset( )
{
    if( BMA253_I2C_Write( BMA2x2_RST_REG, BMA2x2_ENABLE_SOFT_RESET_VALUE, 1 ) == SUCCESS )
    {
        return SUCCESS;
    }
    return FAIL;
...
void BMA253SetDeviceAddr( uint8_t addr )
{
    	I2cDeviceAddr = addr;
}

uint8_t BMA253GetDeviceAddr( void )
{
    	return I2cDeviceAddr;
}

uint8_t BMA253Init( void )
{
	uint8_t regVal = 0;
	BMA253SetDeviceAddr( BMA2x2_I2C_ADDRESS );

	if( BMA253Initialized == false )
	{   
		BMA253Initialized = true;
        
    if(BMA253Reset( ) == FAIL)
    return FAIL;
        
		//read BMA253 id
		BMA253_I2C_Read( BMA2x2_CHIP_ID_REG, &regVal,  1);
		if( regVal != BMA253_ID )
		{
		    return FAIL;
		}
...
void BMA253DeInit( void )
{
    BMA253Initialized = false;
}

uint8_t BMA253_I2C_Write( uint8_t addr, uint8_t data, uint8_t size )
{
	return I2cWriteBuffer( I2cDeviceAddr << 1, addr, &data, size );
}

uint8_t BMA253_I2C_Read( uint8_t addr, uint8_t *data, uint8_t size)
{
	return I2cReadBuffer( I2cDeviceAddr << 1, addr, data, size );
}

int16_t BMA253ConvertToNumerical( uint16_t AxisRaw, uint8_t Res, float base_g )
{
    uint16_t rawdata;
    uint8_t sign_flag = 0;
    int16_t GVal = 0;
    ...
    switch( Res )
    {
        ...
        {
            rawdata = (AxisRaw >> BMA253_10_BIT_DATA_SHIFT);
            sign_flag = (rawdata >> BMA253_10_BIT_SIGN_SHIFT) & 0x01;
            ...
            {
                AxisRaw = ~rawdata;
                AxisRaw = (AxisRaw & BMA253_10_BIT_MASK) + 1;
                GVal = -AxisRaw * base_g;
            }
            break;
        }
 ...
 *              1          | disable MSB Lock
 */
void BMA253_ShadowDis_Disable(bool isDisable)
{
	uint8_t shadow_dis_data;

	BMA253_I2C_Read(BMA2x2_DATA_CTRL_REG, &shadow_dis_data, 1);
	...
	BMA253_I2C_Write(BMA2x2_DATA_CTRL_REG, shadow_dis_data, 1);
}
 ...
 *
*/
void BMA253_Bandwidth_Set(uint8_t bandwidth)
{
	uint8_t bandwidth_data;

	BMA253_I2C_Read(BMA2x2_BW_SELECT_REG, &bandwidth_data, 1);
	...
	BMA253_I2C_Write(BMA2x2_BW_SELECT_REG, bandwidth_data, 1);

}

void BMA253_Range_Set(uint8_t range)
{
	uint8_t range_data;

	BMA253_I2C_Read(BMA2x2_RANGE_SELECT_REG, &range_data, 1);
	...
	BMA253_I2C_Write(BMA2x2_RANGE_SELECT_REG, range_data, 1);
}
 ...
 *
 */
void BMA253_INT_Enable_Set(uint8_t int_type, bool isEnable)
{
	uint8_t int_1_data;
	uint8_t int_2_data;

	BMA253_I2C_Read(BMA2x2_INTR_ENABLE1_REG, &int_1_data, 1);
	BMA253_I2C_Read(BMA2x2_INTR_ENABLE2_REG, &int_2_data, 1);
	...
	BMA253_I2C_Write(BMA2x2_INTR_ENABLE1_REG, int_1_data, 1);
	BMA253_I2C_Write(BMA2x2_INTR_ENABLE2_REG, int_2_data, 1);
 ...
 *
 */
void BMA253_INT_Source_Set(uint8_t channel, uint8_t isEnable)
{
	uint8_t int_source_data;
	BMA253_I2C_Read(BMA2x2_INTR_SOURCE_REG, &int_source_data, 1);
	...
	BMA253_I2C_Write(BMA2x2_INTR_SOURCE_REG, int_source_data, 1);
}
 ...
 *              false              | PUSS_PULL
 */
void BMA253_INT_OutputType_Set(uint8_t channel, bool isOpenDrain)
{
	uint8_t output_data;
	BMA253_I2C_Read(BMA2x2_INTR_SET_REG, &output_data, 1);	
	switch (channel) {
	...
	BMA253_I2C_Write(BMA2x2_INTR_SET_REG, output_data, 1);	
}
 ...
 *
 */
void BMA253_INT_NewData_Set(uint8_t channel, bool isEnable)
{
	uint8_t new_data;


	BMA253_I2C_Read(BMA2x2_INTR_DATA_SELECT_REG, &new_data, 1);	
	switch (channel) {
		/* write the active level */
	...
	BMA253_I2C_Write(BMA2x2_INTR_DATA_SELECT_REG, new_data, 1);		
}
 ...
 *
 */
void BMA253_INT_Level_Set(uint8_t channel, bool isActiveHigh)
{
	uint8_t level_data;
	BMA253_I2C_Read(BMA2x2_INTR_SET_REG, &level_data, 1);	
	switch (channel) {
		/* write the active level */
	...
	BMA253_I2C_Write(BMA2x2_INTR_SET_REG, level_data, 1);	
}
 ...
 */
void BMA253_INT_Latched_Mode_Set(bool isEnable)
{
	uint8_t latched_data;

	BMA253_I2C_Read(BMA2x2_INTR_CTRL_REG, &latched_data, 1);
	if(isEnable)
		latched_data = latched_data | 0x0F;
	else
		latched_data = latched_data & ~0x0F;

	BMA253_I2C_Write(BMA2x2_INTR_CTRL_REG, latched_data, 1);
}
 ...
 *              0x2              |  y axis
  *             0x3              |  z axis
 */
void BMA253_INT_Compensation_Set(uint8_t channel)
{
	...
	while(cal_rdy == 0x0) {
		BMA253_I2C_Read(BMA2x2_OFFSET_CTRL_REG, &offset_ctrl_data, 1);
		cal_rdy = (offset_ctrl_data & 0x10) >> BMA253_FFSET_COMPENSATION_CAL_RTY_SHIFT;
//		DelayMs(10);
	}
	offset_ctrl_data = (offset_ctrl_data & BMA253_FFSET_COMPENSATION_CAL_TRIGGER_MASK) \
        | ((channel & 0x3) << BMA253_FFSET_COMPENSATION_CAL_TRIGGER_SHIFT);
    
	BMA253_I2C_Write(BMA2x2_OFFSET_CTRL_REG, offset_ctrl_data, 1);
 ...
 */
void BMA253_INT_RST(bool isReset)
{
	uint8_t reset_data;

	BMA253_I2C_Read(BMA2x2_INTR_CTRL_REG, &reset_data, 1);
	if(isReset)
		reset_data = reset_data | 0x80;
	else
		reset_data = reset_data & ~0x80;

	BMA253_I2C_Write(BMA2x2_INTR_CTRL_REG, reset_data, 1);
}
 ...
 *
 */
void BMA253_INT_Slope_Set(uint8_t channel, bool isEnable)
{
	uint8_t slope_data;
	...
	switch (channel) {
		...
		case BMA2x2_INTR1_SLOPE:
			BMA253_I2C_Read(BMA2x2_INTR1_PAD_SELECT_REG, &slope_data, 1);	
			if(isEnable) {
				slope_data = slope_data | 0x4;
			} else {
				slope_data = slope_data & ~0x4;
			}
			BMA253_I2C_Write(BMA2x2_INTR1_PAD_SELECT_REG, slope_data, 1);	
			break;
		case BMA2x2_INTR2_SLOPE:
			BMA253_I2C_Read(BMA2x2_INTR2_PAD_SELECT_REG, &slope_data, 1);	
			if(isEnable) {
				slope_data = slope_data | 0x4;
			} else {
				slope_data = slope_data & ~0x4;
			}
			BMA253_I2C_Write(BMA2x2_INTR2_PAD_SELECT_REG, slope_data, 1);	
			break;
 ...
 *
 */
void BMA253_INT_Slope_Threshold_Set(uint8_t channel, uint8_t threshold)
{
	...
	switch (channel) {
		...
		case BMA2x2_LOW_THRES:
			...
			BMA253_I2C_Write(BMA2x2_LOW_THRES_REG, threshold_data, 1);
			break;
		case BMA2x2_HIGH_THRES:
			/*HIGH THRESHOLD*/
			threshold_data = threshold;
			BMA253_I2C_Write(BMA2x2_HIGH_THRES_REG, threshold_data, 1);
			break;
		case BMA2x2_SLOPE_THRES:
			/*SLOPE THRESHOLD*/
			threshold_data = threshold;
			BMA253_I2C_Write(BMA2x2_SLOPE_THRES_REG, threshold_data, 1);
			break;
		case BMA2x2_SLOW_NO_MOTION_THRES:
			/*SLO NO MOT THRESHOLD*/
			threshold_data = threshold;
			BMA253_I2C_Write(BMA2x2_SLOW_NO_MOTION_THRES_REG, threshold_data, 1);
			break;
		default:
			break;
	}
 ...
 *
 */
void BMA253_INT_Slope_Duration_Set(uint8_t channel, uint8_t duration)
{
	...
	switch (channel)   {
		case BMA2x2_LOW_DURN:
			...
			BMA253_I2C_Write(BMA2x2_LOW_DURN_REG, duration_data, 1);
			break;
		case BMA2x2_HIGH_DURN:
			/*HIGH DURATION*/
			duration_data = duration;
			BMA253_I2C_Write(BMA2x2_HIGH_DURN_REG, duration_data, 1);
			break;
		case BMA2x2_SLOPE_DURN:
			/*SLOPE DURATION*/
			BMA253_I2C_Write(BMA2x2_SLOPE_DURN_REG, duration_data, 1);
...
void BMA253_INT_Slope_Hysteresis_Set(uint8_t channel, uint8_t hysteresis)
{
...
//			BMA253_I2C_Write(BMA2x2_LOW_HYST_REG, hysteresis_data, 1);
            BMA253_I2C_Read(BMA2x2_LOW_HYST_REG, &hysteresis_data, 1);
 ...
 *              true              | INTR_ENABLE
*/
void BMA253_INT_Orient_Set(uint8_t channel, uint8_t isEnable)
{
	uint8_t orient_data;
	...
	switch (channel) {
		...
		case BMA2x2_INTR1_ORIENT:
			BMA253_I2C_Read(BMA2x2_INTR1_PAD_SELECT_REG, &orient_data, 1);	
			if(isEnable) {
				orient_data = orient_data | 0x40;
			} else {
				orient_data = orient_data & ~0x40;
			}
			BMA253_I2C_Write(BMA2x2_INTR1_PAD_SELECT_REG, orient_data, 1);	
			break;
		case BMA2x2_INTR2_ORIENT:
			BMA253_I2C_Read(BMA2x2_INTR2_PAD_SELECT_REG, &orient_data, 1);	
			if(isEnable) {
				orient_data = orient_data | 0x40;
			} else {
				orient_data = orient_data & ~0x40;
			}
			BMA253_I2C_Write(BMA2x2_INTR2_PAD_SELECT_REG, orient_data, 1);	
			break;
		default:
			break;
	}	
 ...
 *
*/
void BMA253_INT_Orient_Mode_Set(uint8_t mode)
{
	uint8_t orient_mode_data;
	BMA253_I2C_Read(BMA2x2_ORIENT_PARAM_REG, &orient_mode_data, 1);	
	switch (mode) {
	...
	BMA253_I2C_Write(BMA2x2_ORIENT_PARAM_REG, orient_mode_data, 1);
}
 ...
 *
 */
void BMA253_INT_Orient_Block_Mode_Set(uint8_t mode)
{
	uint8_t block_mode_data;

	BMA253_I2C_Read(BMA2x2_ORIENT_PARAM_REG, &block_mode_data, 1);	
	switch (mode) {
		case 0x0:
	...
	BMA253_I2C_Write(BMA2x2_ORIENT_PARAM_REG, block_mode_data, 1);
}
 ...
 *
 */
void BMA253_INT_Orient_Block_Threshold_Set(double deg_angle)
{	
	double rad_angle = 0;
	...
	BMA253_I2C_Read(BMA2x2_THETA_BLOCK_REG, &orient_threshold_data, 1);	
	orient_threshold_data = (orient_threshold_data & 0xC0) | threshold_theta;
	BMA253_I2C_Write(BMA2x2_THETA_BLOCK_REG, orient_threshold_data, 1);
}
 ...
 */
void BMA253_INT_Orient_Hyst_Set(uint8_t flat_hy)
{
	uint8_t orient_hyst_data;

	BMA253_I2C_Read(BMA2x2_ORIENT_PARAM_REG, &orient_hyst_data, 1);	
	orient_hyst_data = (orient_hyst_data & 0x8F) | ((flat_hy & 0x7) << 4);
	BMA253_I2C_Write(BMA2x2_ORIENT_PARAM_REG, orient_hyst_data, 1);	
}
 ...
 *              true              | INTR_ENABLE
*/
void BMA253_INT_Flat_Set(uint8_t channel, uint8_t isEnable)
{
	uint8_t flat_data;
	...
	switch (channel) {
		...
		case BMA2x2_INTR1_FLAT:
			BMA253_I2C_Read(BMA2x2_INTR1_PAD_SELECT_REG, &flat_data, 1);	
			if(isEnable) {
				flat_data = flat_data | 0x80;
			} else {
				flat_data = flat_data & ~0x80;
			}
			BMA253_I2C_Write(BMA2x2_INTR1_PAD_SELECT_REG, flat_data, 1);	
			break;
		case BMA2x2_INTR2_FLAT:
			BMA253_I2C_Read(BMA2x2_INTR2_PAD_SELECT_REG, &flat_data, 1);	
			if(isEnable) {
				flat_data = flat_data | 0x80;
			} else {
				flat_data = flat_data & ~0x80;
			}
			BMA253_I2C_Write(BMA2x2_INTR2_PAD_SELECT_REG, flat_data, 1);	
			break;
		default:
			break;
	}	
 ...
 *
 */
void BMA253_INT_Flat_Block_Threshold_Set(double deg_angle)
{	
	double rad_angle = 0;
	...
	BMA253_I2C_Read(BMA2x2_THETA_FLAT_REG, &flat_threshold_data, 1);	
	flat_threshold_data = (flat_threshold_data & 0xC0) | threshold_theta;
	BMA253_I2C_Write(BMA2x2_THETA_FLAT_REG, flat_threshold_data, 1);
}
 ...
 *
 */
 void BMA253_INT_Flat_Hold_Time_Set(uint8_t hold_time)
{
	uint8_t flat_hold_time_data;

	BMA253_I2C_Read(BMA2x2_FLAT_HOLD_TIME_REG, &flat_hold_time_data, 1);	
	flat_hold_time_data = (flat_hold_time_data & 0xCF) | ((hold_time & 0x3) << 4);
	BMA253_I2C_Write(BMA2x2_FLAT_HOLD_TIME_REG, flat_hold_time_data, 1);	
}
 ...
 *  @param flat_hy : The value of flat hysteresis
 */
void BMA253_INT_Flat_Hyst_Set(uint8_t flat_hy)
{
	uint8_t flat_hyst_data;

	BMA253_I2C_Read(BMA2x2_FLAT_HOLD_TIME_REG, &flat_hyst_data, 1);	
	flat_hyst_data = (flat_hyst_data & 0xF8) | (flat_hy & 0x7);
	BMA253_I2C_Write(BMA2x2_FLAT_HOLD_TIME_REG, flat_hyst_data, 1);	
}
...
void BMA253_INT_Status0_Get(void)
{
    uint8_t status0_data;
	BMA253_I2C_Read(BMA2x2_STAT1_REG, &status0_data, 1);
}

void BMA253_INT_Status2_Get(void)
{
    uint8_t status2_data;
	BMA253_I2C_Read(BMA2x2_STAT_TAP_SLOPE_REG, &status2_data, 1);
}

void BMA253_Polling_Data(struct BMA253_Accel_Data *data)
{
	uint8_t	data_lsb[2] = {0, 0};
//	uint8_t	data_msb = 0;
    int16_t rawdata = 0;
	uint8_t 	range = BMA253_RANGE;
	float 	base_g = 0;
	...
	switch (BMA253_RESOLUTION) {
		...
		case BMA2x2_10_RESOLUTION:
			BMA253_I2C_Read(BMA2x2_ACCEL_X10_LSB__REG, data_lsb, 2);
            rawdata = (int16_t)((((int32_t)((uint8_t)data_lsb[1])) << 8) |(data_lsb[0] & RESOLUTION_10_MASK));
            data->new_data_x = data_lsb[0] & BMA253_10_NEW_DATA_MASK;
			data->x_lsb = BMA253ConvertToNumerical(rawdata, BMA2x2_10_RESOLUTION, base_g);

			BMA253_I2C_Read(BMA2x2_ACCEL_Y10_LSB__REG, data_lsb, 2);
            rawdata = (int16_t)((((int32_t)((uint8_t)data_lsb[1])) << 8) |(data_lsb[0] & RESOLUTION_10_MASK));
            data->new_data_y = data_lsb[0] & BMA253_10_NEW_DATA_MASK;
			data->y_lsb = BMA253ConvertToNumerical(rawdata, BMA2x2_10_RESOLUTION, base_g);

            BMA253_I2C_Read(BMA2x2_ACCEL_Z10_LSB__REG, data_lsb, 2);
            rawdata = (int16_t)((((int32_t)((uint8_t)data_lsb[1])) << 8) |(data_lsb[0] & RESOLUTION_10_MASK));
            data->new_data_z = data_lsb[0] & BMA253_10_NEW_DATA_MASK;
			data->z_lsb = BMA253ConvertToNumerical(rawdata, BMA2x2_10_RESOLUTION, base_g);
            
			break;

		/* This case used for the resolution bit 14*/
		case BMA2x2_14_RESOLUTION:
			BMA253_I2C_Read(BMA2x2_ACCEL_X14_LSB__REG, data_lsb, 2);
			data->x_lsb = (int16_t)((((int32_t)((uint8_t)data_lsb[1])) << 8) |(data_lsb[0] & RESOLUTION_14_MASK));
			data->x_lsb= (int16_t)((data->x_lsb >> 2) * base_g);

			BMA253_I2C_Read(BMA2x2_ACCEL_Y14_LSB__REG, data_lsb, 2);
			data->y_lsb = (int16_t)((((int32_t)((uint8_t)data_lsb[1])) << 8) |(data_lsb[0] & RESOLUTION_14_MASK));
			data->y_lsb= (int16_t)((data->y_lsb >> 2) * base_g);

			BMA253_I2C_Read(BMA2x2_ACCEL_Z14_LSB__REG, data_lsb, 2);
...
void BMA253_Polling_Compensation_Offset(uint8_t *ofx, uint8_t *ofy, uint8_t *ofz)
{
	BMA253_I2C_Read(BMA2x2_OFFSET_X_AXIS_REG, ofx, 1);
	BMA253_I2C_Read(BMA2x2_OFFSET_Y_AXIS_REG, ofy, 1);
	BMA253_I2C_Read(BMA2x2_OFFSET_Z_AXIS_REG, ofz, 1);
}

void BMA253_Mode_Setting(uint8_t mode)
{
    uint8_t reg_data;
    BMA253_I2C_Read(BMA253_PMU_LPW_REG, &reg_data, 1);
    reg_data = (reg_data & BMA253_MODE_MASK) | mode;
    BMA253_I2C_Write(BMA253_PMU_LPW_REG, reg_data, 1);
}

void BMA253_Sleep_During_Setting(uint8_t time)
{
    uint8_t reg_data;
    BMA253_I2C_Read(BMA253_PMU_LPW_REG, &reg_data, 1);
    reg_data = (reg_data & BMA253_SLEEP_DURING_MASK) | time;
    BMA253_I2C_Write(BMA253_PMU_LPW_REG, reg_data, 1);
}

void BMA253_Low_Power_Mode(uint8_t mode)
{
    uint8_t reg_data;
    BMA253_I2C_Read(BMA253_PMU_LOW_POWER_REG, &reg_data, 1);
    reg_data = (reg_data & BMA253_LOW_POWER_MODE_MASK) | (mode << BMA253_LOW_POWER_MODE_BIT);
    BMA253_I2C_Write(BMA253_PMU_LOW_POWER_REG, reg_data, 1);
}

void BMA253_Low_Power_Sleep_Timer_Mode(uint8_t mode)
{
    uint8_t reg_data;
    BMA253_I2C_Read(BMA253_PMU_LOW_POWER_REG, &reg_data, 1);
    reg_data = (reg_data & BMA253_LOW_POWER_SLEEP_TIMER_MODE_MASK) | (mode << BMA253_LOW_POWER_SLEEP_TIMER_MODE_BIT);
    BMA253_I2C_Write(BMA253_PMU_LOW_POWER_REG, reg_data, 1);
}

bool BMA253_Flat_Status(void)
{
    uint8_t reg_data;
    BMA253_I2C_Read(BMA253_STAT_FLAT_ORIENT_HIGH_REG, &reg_data, 1);
    if((reg_data&BMA253_INT_STATUS_FLAT_MASK) == BMA253_INT_STATUS_FLAT_MASK)
        return true;
    else
        return false;
}

void BMA253_Setting_Fast_Compensation_Target(uint8_t x, uint8_t y, uint8_t z)
{
    uint8_t reg_data;
    BMA253_I2C_Read(BMA253_OFC_SETTING_REG, &reg_data, 1);
    reg_data = reg_data & BMA253_FFSET_COMPENSATION_MASK;
    reg_data = (reg_data | x << BMA253_FFSET_COMPENSATION_X_BIT);
    reg_data = (reg_data | y << BMA253_FFSET_COMPENSATION_Y_BIT);
    reg_data = (reg_data | z << BMA253_FFSET_COMPENSATION_Z_BIT);
    BMA253_I2C_Write(BMA253_OFC_SETTING_REG, reg_data, 1);
}
